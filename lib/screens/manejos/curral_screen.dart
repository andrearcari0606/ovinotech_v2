...
class _CurralScreenState extends State<CurralScreen> {
 ...
 void carregarPesoNoCampo(Animal animal) {
  final manejo = getAtual(animal);
  final resumo = carregarResumoAnimal(animal);

  final peso = manejo.peso ?? resumo.peso;
  pesoController.text = peso != null ? peso.toString() : '';
  }
  ...
  void initState() {
    ...
    if (lista.isNotEmpty) {
      ...
      carregarPesoNoCampo(animalSelecionado!);
    }
  }
  ...
  // 🔥 GARANTE QUE PEGA DO CAMPO
  final pesoDigitado = double.tryParse(pesoController.text);
  if (pesoDigitado != null) {
    manejo.peso = pesoDigitado;
  }
  
  if (manejo.peso == null) {
  print("❌ NÃO SALVOU - CAMPO VAZIO");
  return;
  }
  ...
  // 🔥 SALVAR PESO
  if (manejo.peso != null) {
    await HiveService.pesagemBox.add(
     Pesagem(
    ...
    print("✅ SALVOU PESO: ${manejo.peso}");
  } else {
    print("❌ NÃO SALVOU - PESO NULL");
  }

  // 🔥 SALVAR FAMACHA
  if (manejo.famacha != null) {
    await HiveService.famachaBox.add(
     Famacha(
  ...
  Widget build(BuildContext context) {
    ...
    final manejoAtual = getAtual(animal);
    final resumo = carregarResumoAnimal(animal);

    final combinado = ManejoTemp()
      ..famacha = manejoAtual.famacha ?? resumo.famacha
      ..ecc = manejoAtual.ecc ?? resumo.ecc
      ..peso = manejoAtual.peso ?? resumo.peso;

    final statusCor = corStatus(combinado);

    return Scaffold(
      appBar: AppBar(
        title: Text("Curral (${indexAtual + 1}/${lista.length})"),
      ),
      body: Padding(
          ...
          children: [
            ...
            Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text("Brinco ${animal.identificacao}",
                      style: const TextStyle(
                          fontSize: 20, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 6),
                  Text("Peso: ${combinado.peso ?? '-'} kg"),
            ...
            Row(
              children: List.generate(5, (i) {
                ...
                final selecionado = manejoAtual.famacha == valor;

                return Expanded(
                  child: GestureDetector(
                    onTap: () {
                      setState(() {
                        manejoAtual.famacha = valor;
                        manejoAtual.alterado = true;
                      });
                    },
                    child: Container(
                      ...
                      child: Center(
                        child: Text(valor.toString(),
                            style: TextStyle(
                              color: valor >= 4 ? Colors.black : Colors.white,
            ...
            Row(
              children: List.generate(5, (i) {
                ...
                final selecionado = manejoAtual.ecc == valor;

                return Expanded(
                  child: GestureDetector(
                    onTap: () {
                      setState(() {
                        manejoAtual.ecc = valor;
                        manejoAtual.alterado = true;
                      });
                    },
                    child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        color: cor.withOpacity(selecionado ? 0.85 : 0.3),
                        borderRadius: BorderRadius.circular(8),
                        border: selecionado
            ...
            TextField(
             ...
             if (m.peso != null && m.peso != animal.peso) {
